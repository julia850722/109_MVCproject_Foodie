...
namespace Foodie.Controllers
{
    ...
    public class PurchaseOffersController : Controller
    {
        ...
        [HttpPost]
        public ActionResult RequestIndex(FormCollection fc, string searchString)
        {
            ViewBag.PostToInfo = searchString;
            return View();
        }

        public ActionResult Info(string search)
        {
            if (!String.IsNullOrEmpty(search))
            {
                return PartialView(db.PurchaseOffers.Where(p => p.OfferedRestaurant == search).ToList());
            }
            return PartialView(db.PurchaseOffers.OrderByDescending(p => p.Seller.Id).ToList());
        }
